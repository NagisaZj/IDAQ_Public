...
argmin = np.argmin(all_paths_uncertainties[0])
...
#         #
#         plt.plot(states[:-1, 0], states[:-1, 1],':' if i!=0 else '',color='g' if i!=0 else np.array([0.9, 0.17, 0.31]),linewidth=5 if i<10 else 2)
#
...
#     data = pickle.load(f)
    ...
    for i in range(0,20):#np.array([0.9, 0.17, 0.31])
        if i < 10 or i == (m + 10):
            states = all_paths[0][i]
            plt.plot(states[:-1, 0], states[:-1, 1],':' if i !=argmin else '', color= 'g' ,linewidth= 2)
        if i==m+10:
            plt.plot(states[:-1, 0], states[:-1, 1]-0.02,  '',
                     color= np.array([0.9, 0.17, 0.31]), linewidth=2)
    # plt.plot(states[-1, 0], states[-1, 1], '-x', markersize=10, color=colors[0])
    # plt.set(aspect='equal')
#
# for i in range(10,20):
