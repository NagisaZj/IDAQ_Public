...
for i in range(10,20):
    ...
    plt.plot(states[:-1, 0], states[:-1, 1], '', color=np.array([1.0, 0.49, 0.0]),linewidth=3)
print(data)
for i in range(0, 10):
    states = data[i]['observations']
    plt.plot(states[:-1, 0], states[:-1, 1], '', color='r', linewidth=3)
...
for i in range(1):
    ...
    plt.scatter(positions[:,0],positions[:,1],s=100,color=np.array([0.66, 0.66, 0.66]),alpha=0.2)
plt.xlim(-1.5,2.0)
plt.ylim(-0.5,2.0)
...
for g in goals:
    ...
    plt.plot(g[0], g[1],'x',markersize=30,color='b')
