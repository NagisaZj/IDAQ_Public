...
def experiment(variant, cfg=None, goal_idx=0, seed=0,  eval=False):
    os.makedirs('./data/'+variant['env_name']+'2/goal_idx%d'%goal_idx,exist_ok=True)
    ...
    model = SAC("MlpPolicy", env, verbose=1, tensorboard_log="./sac_mt1/")
    # model.learn(total_timesteps=1000000, log_interval=4)
    model.load('window-open-v2_111')
    ...
    while success_cnt <45:
        ...
        if 1:
            ...
            np.save(os.path.join('./data/'+variant['env_name']+'2/goal_idx%d'%goal_idx, f'trj_evalsample{success_cnt}_step{49500}.npy'), np.array(trj))
            success_cnt+=1
        else:
...
def main(config, num_gpus, docker, debug, eval, is_uniform, goal_idx=0, seed=0):
    ...
    # multi-processing
    p = mp.Pool(min(mp.cpu_count(), 1))

    os.makedirs('./data/'+variant['env_name'],exist_ok=True)
    if variant['env_params']['n_tasks'] > 1:
        p.starmap(experiment, product([variant], [None], random_task_id))
    else:
