...
#     data = pickle.load(f)
    baseline_return = all_paths_returns[0][8]-0.01
    for i in range(0,20):
        if i<(m+10):
            states = all_paths[0][i]
            if all_paths_returns[0][i]>baseline_return:
                format=''
                baseline_return = all_paths_returns[0][i]
            else:
                format = ':'
            plt.plot(states[:-1, 0], states[:-1, 1],format, color='g' ,linewidth=1.5)
            # plt.plot(states[:-1, 0], states[:-1, 1], '', color='g' ,linewidth=2)
        if  i == (m + 10):
            states = all_paths[0][i]
            plt.plot(states[:-1, 0]-0.015, states[:-1, 1],  '',
                     color= np.array([0.9, 0.17, 0.31]), linewidth= 1.5)
